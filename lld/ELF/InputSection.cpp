...
void InputSection::copyRelocations(Ctx &ctx, uint8_t *buf) {
  if ((ctx.arg.relax && !ctx.arg.relocatable &&
      ...
      ((ctx.arg.relax || ctx.arg.expand) && !ctx.arg.relocatable &&
...
void EhInputSection::split(ArrayRef<RelTy> rels) {
  ...
  while (!d.empty()) {
    ...
    SyntheticSection *s = getParent();
    if (s->ctx.arg.emachine == EM_NANOMIPS) {
      ...
      if (relsNotLessThanOffset) {
        for (auto [i, rel] : llvm::enumerate(*relsNotLessThanOffset)) {
          ...
          RelType type = rel.getType(s->ctx.arg.isMips64EL);
          RelExpr expr = s->ctx.target->getRelExpr(type, sym, 0);
